...
const cols = [
  { title: 'Services', links: ['SEO & Content','Paid Media','Web Design','Branding','Cloud Services'] },
...
export default function Footer() {
  return (
    <footer className="border-t border-black/10 bg-ash px-8 lg:px-16 pt-20 pb-10">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-16">
        <div className="lg:col-span-2">
          <div className="font-display font-extrabold text-2xl mb-4 text-ink" style={{ fontFamily: 'var(--font-syne)' }}>
        ...
        {cols.map(col => (
          <div key={col.title}>
            ...
            <ul className="space-y-3">
              {col.links.map(l => (
                <li key={l}>
                  <Link href="#" className="font-mono text-sm text-ink/60 hover:text-ink transition-colors duration-150 u-draw">{l}</Link>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between pt-8 border-t border-black/10 font-mono text-xs text-mute gap-4">
